...
//URL, WIFI
String url = "123.143.18.211";  //데이터 수신 및 송신에 쓰이는 URL
String port = "33333";  //데이터 수신 및 송신에 쓰이는 PORT
String ssid = "6-304";    //wifi ssid
...
void soilHumidRun(){            // 토양 습도 센서
  ...
  else Serial.println("\nWater pump has been disabled. Please check hardware\n");
}
  ...
  if(!flag){
    while(true){
      ...
      if((endTime - startTime) > 5000){     // 펌프가 5초 이상 작동되면 강제종료
        digitalWrite(pump, LOW);
        Serial.println("\n==========================================");
        Serial.println("ERROR!  Pump malfunction or lack of water.");
        Serial.println("Supply Cancelled. Please check hardware.");
        Serial.println("==========================================");
        errCodeSave("pumpErr");
        pumpState = false;
        break;
